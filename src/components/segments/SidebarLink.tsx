...
function toggleSublinks(id: number): void {
  ...
  sidebarSublinks?.classList.contains("absolute") ? toggleAbsolute() : setTimeout(() => toggleAbsolute(), 200);
}

function Item({ name, Icon }: ItemTemplate) {
  return <div className="p-1.5 justify-center m-1.5 flex items-center hoverable-btn capitalize">
    {Icon}
    <h6>{name}</h6>
  </div>
}
...
function SidebarLink(link: LinkTemplate) {
  ...
  return <div className="text-xl md:text-2xl font-bold text-skin-base w-full border-crust border-b-2">
    {(sublinks === undefined) ? <a href={link.url}><Item {...link} /></a> : <div>
      ...
      <ul id={`sidebar-sublinks-${id}`} className="ml-1 flex-col absolute scale-y-0 transition-transform origin-top ease-linear">
        {sublinks.map(sublink => <li key={sublink.id}>
